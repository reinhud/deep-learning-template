...
class CIFAR100DataModule(L.LightningDataModule):
    ...
    def __init__(
        ...
        """Initializes the CIFAR100DataModule.

        Args:
            batch_size (int): The batch size for data loaders.
            data_dir (str): The directory to store CIFAR100 dataset.
    ...
    def prepare_data(self) -> None:
        ...
        try:
            ...
            self.class_to_idx = CIFAR100(self.data_dir, train=False, download=True).class_to_idx
        except Exception as e:
            raise RuntimeError(f"Failed to prepare CIFAR100 dataset: {e}")
    ...
    def setup(self, stage: str = None) -> None:
        ...
        if stage == "fit" or stage is None:
            cifar100_full = CIFAR100(self.data_dir, train=True, transform=transform_train)
        ...
        if stage == "test" or stage is None:
            self.cifar100_test = CIFAR100(self.data_dir, train=False, transform=transform_test)

        if stage == "predict" or stage is None:
            self.cifar100_predict = CIFAR100(self.data_dir, train=False, transform=transform_test)
